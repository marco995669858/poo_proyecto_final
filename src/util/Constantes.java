package util;

public class Constantes {
	//VARIABLES ESTATICAS REUTILIZABLES
	public static String[] COLUMNAS_CURSOS = {"ID", "Nombre del Curso", "Crédito"};
	
	public static String[] COLUMNAS_DOCENTES = {"ID", "Dni", "Apellidos", "Nombres", "Especialidad", "Dirección", "Telefono"};
	
	public static String[] COLUMNAS_ESTUDIANTES = {"ID", "Dni", "Año de ingreso", "Escuela", "Facultada", "Apellidos", "Nombres", "Dirección", "Telefono"};
	
	public static String[] COLUMNAS_ENCARGADO_MATRICULA= {"ID", "Cargo", "Dni", "Dirección", "Apellidos", "Nombres", "Telefono"};
	
	public static String USUARIO = "75058989";
	
	//PARA HACER EL CASO DEL GIT REVERT DE EJEMPLO PRUEBA WORD 
}
